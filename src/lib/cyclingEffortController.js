...
const formatSegmentDebugTooltipHtml = (segment, index) => {
    ...
    const windDirectionLabel =
        segment.windDirectionFrom == null ? 'n/a' : `${round(segment.windDirectionFrom, 0)}°`;
    const headingLabel =
...
const isCyclingOverlayLayer = layer =>
    Boolean(layer?.__cyclingEffortOverlay || layer?.options?.className?.includes(OVERLAY_CLASS));
    ...
    new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = evt => {
            ...
            if (!db.objectStoreNames.contains(ELEVATION_STORE_NAME)) {
                db.createObjectStore(ELEVATION_STORE_NAME, { keyPath: 'key' }).createIndex(
                    'storedAt',
                    'storedAt',
                );
            ...
            if (!db.objectStoreNames.contains(WEATHER_STORE_NAME)) {
                db.createObjectStore(WEATHER_STORE_NAME, { keyPath: 'key' }).createIndex(
                    'storedAt',
                    'storedAt',
                );
            } else if (evt.oldVersion < 3) {
...
const putElevationInDB = (db, key, value) => {
    try {
        db.transaction(ELEVATION_STORE_NAME, 'readwrite')
            .objectStore(ELEVATION_STORE_NAME)
...
const putWeatherInDB = (db, key, value) => {
    try {
        db.transaction(WEATHER_STORE_NAME, 'readwrite')
            .objectStore(WEATHER_STORE_NAME)
            .put({ key, value, storedAt: Date.now() });
    } catch (_e) {
        // Ignore storage errors
...
class CyclingEffortController {
    constructor() {
        ...
        this.elevationDB = openElevationDB()
            .then(db => {
    ...
    isPotentialRouteMutationLayer(layer) {
        ...
        if (typeof layer.getLatLngs === 'function' || typeof layer.toGeoJSON === 'function') {
            return true;
        }
    ...
    collectTrackCandidates() {
        ...
        const walkLayer = layer => {
            if (layer == null || seenLayers.has(layer) || isCyclingOverlayLayer(layer)) {
                return;
            }
            seenLayers.add(layer);

            if (typeof layer.eachLayer === 'function' && typeof layer.getLayers === 'function') {
        ...
        const uploaded = candidates.filter(candidate => candidate.looksLikeUpload);
    ...
    collectStylableRouteLayers(layer, output = []) {
        ...
        if (typeof layer.eachLayer === 'function' && typeof layer.getLayers === 'function') {
            layer.eachLayer(child => this.collectStylableRouteLayers(child, output));
        }
        if (typeof layer.setStyle === 'function' && typeof layer.getLatLngs === 'function') {
            output.push(layer);
        }
        return output;
    }
    ...
    async fetchAccurateWindForPoints(points) {
        ...
        const maxWindSamples = Math.max(
            1,
            Math.round(MAX_ACCURATE_WIND_SAMPLES_PER_KM * routeLengthKm),
        );
            ...
            await Promise.allSettled(
                batch.map(async idx => {
                    ...
                    try {
                        ...
                        const wind = this.parseWindFromForecastPayload(
                            httpPayload,
                            targetTimestamp,
                        );
                        if (wind != null) {
    ...
    createWeatherSampler(accurateWindData) {
        const entries =
            accurateWindData != null && accurateWindData.size > 0
    ...
    async analyze(points) {
        ...
        const weatherScaling = weatherWeight / Math.max(1e-3, WEATHER_WEIGHT_BASELINE);
        const elevationScaling = elevationWeight / Math.max(1e-3, ELEVATION_WEIGHT_BASELINE);
        ...
        for (let i = 1; i < pointsWithElevation.length; i += 1) {
            ...
            const headCrosswindEffectiveSpeed = headCrosswindSpeed + headCrosswindGustSpeed;
            const tailCrosswindEffectiveSpeed = tailCrosswindSpeed + tailCrosswindGustSpeed;
            ...
            const descentPenalty = descent * SEGMENT_EFFORT_MODIFIERS.descentPenaltyPerMeter;
            const downhillReliefRaw = descent * SEGMENT_EFFORT_MODIFIERS.descentReliefPerMeter;
            const maxDownhillRelief = baseEffort * SEGMENT_EFFORT_MODIFIERS.maxDownhillReliefShare;
            ...
            const weatherPenaltyComponent = (headwindEffort + headCrosswindEffort) * weatherScaling;
        ...
        for (const [index, segment] of segments.entries()) {
            const relativeIntensityNorm =
                spread > 1e-6 ? (intensityBySegment[index] - minIntensity) / spread : 0.5;
            ...
            const windDifficultyNorm = clamp(
                headwindNorm * SEGMENT_WIND_FACTORS.headwindDifficultyWeight +
                    headCrosswindNorm * SEGMENT_WIND_FACTORS.headCrosswindDifficultyWeight +
            ...
            const gradientContribution = gradientDifficultyNorm * domainWeights.terrain;
            ...
            const terrainColorShift = uphillNorm * 0.3 - downhillNorm * 0.2;
            // Wind: headwind pushes toward red, tailwind pushes toward green.
            // Higher wind speeds = progressively stronger effect.
            const adverseWindNorm = headwindNorm * 1.0 + headCrosswindNorm * 0.72 + gustNorm * 0.55;
            const assistiveWindNorm = tailwindNorm * 1.0 + tailCrosswindNorm * 0.4;
            const windColorShift = (adverseWindNorm - assistiveWindNorm) * 0.42;
            const colorDifficulty = clamp(0.5 + terrainColorShift + windColorShift, 0, 1);
            segment.normalizedEffort = colorDifficulty;
            segment.color = colorForSegmentDifficulty(colorDifficulty, intensityNorm, downhillNorm);
            segment.debug = {
                normalizedEffort,
        ...
        const routeGraphSegments = segments.map(segment => ({
            ...
            startElevationM: safeNumber(segment.start?.ele ?? segment.end?.ele ?? 0) ?? 0,
            endElevationM: safeNumber(segment.end?.ele ?? segment.start?.ele ?? 0) ?? 0,
            color: segment.color,
        }));
    ...
    openSegmentDebugPopup(segment, index, latlng, { pin = false } = {}) {
        if (!SHOW_SEGMENT_DEBUG_TOOLTIPS || typeof window === 'undefined' || !window.L || !latlng) {
            return;
        }
        try {
    ...
    renderSegments(segments) {
        ...
        for (const [index, segment] of segments.entries()) {
            ...
            if (SHOW_SEGMENT_DEBUG_TOOLTIPS && typeof polyline.on === 'function') {
                ...
                const onHover = evt =>
                    this.openSegmentDebugPopup(segment, index, evt?.latlng ?? fallbackLatLng);
                const onClick = evt => {
    ...
    async recompute(reason) {
        ...
        try {
            ...
            const routePoints = this.isRouteInverted ? [...chosen.points].reverse() : chosen.points;
            const computePoints = downsamplePoints(routePoints, MAX_POINTS_FOR_COMPUTE);
